    ...
    def perform_step(self, params: dict, output: WorkflowOutput):
        ...
        for file, casp in params["input"].items():
            ...
            if casp not in self.casps:
                logger.error(
                    f"{casp} is not a valid casp name. Available casps are:"
                    f" {list(self.casps.keys())}"
                )
                raise KeyError
        ...
        if params["functionalities"] is not None:
            ...
            for request in requests:
                if request not in self.steps_map:
                    logger.error(
                        f'"{request}" is not a valid functionality. Available'
                        f" functionalities are: {list(self.steps_map.keys())}"
                    )
                    raise KeyError
    ...
    if verbose:
        ...
        return {
            "input_dict": {
                ...
                "choices": None,
                "help": (
                    "Path to the input files and relative casp names in the form"
                    ' "file_path"="casp_name". Available CASP names are {}'.format(
            ...
            "mapping": {
                ...
                "choices": [True, False],
                "help": (
                    "Whether the atom-to-atom mapping of the reactions should be"
                    " performed"
                ),
                "dest": "mapping",
            },
            "mapper": {
                ...
                "help": (
                    "Which mapper should be used; if None, the mapping pipeline is used"
                ),
                "dest": "mapper",
            },
            "functionalities": {
            ...
            "descriptors": {
                ...
                "help": (
                    "List of descriptors to be calculated; if None, all descriptors are"
                    " calculated"
                ),
                "dest": "descriptors",
            },
            ...
            "ged_params": {
                ...
                "help": (
                    "Parameters of the molecular and/or reaction chemical similarity"
                ),
                "dest": "ged_params",
            },
            "clustering_method": {
            ...
            "n_cpu": {
                "name_or_flags": ["-n_cpu"],
                "default": (
                    'All the available CPUs as given by "multiprocessing.cpu_count()"'
                ),
                "required": False,
                "type": int,
                "choices": None,
