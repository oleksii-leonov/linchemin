    ...
    def compute_ged(
        ...
        if (
            ...
            logger.error(
                ...
                f" {type(syngraph2)}. The GED cannot be computed between graph of"
                " different types."
            )
...
class GedFactory:
    ...
    available_ged = {
        ...
        "nx_ged_matrix": {
            ...
            "info": (
                "Standard NetworkX GED algorithm. The distance matrix is computed in"
                ' advanceand the "root" algorithm is used'
            ),
        },
        "nx_optimized_ged": {
            "value": GedOptNx,
    ...
    def select_ged(
        ...
        if ged_method not in self.available_ged:
            logger.error(
                f"'{ged_method}' is invalid. Available algorithms are:"
                f" {self.available_ged.keys()}"
            )
...
def compute_distance_matrix(
    ...
    if len(syngraphs) < 2:
        logger.error(
            "Less than 2 routes were found: it is not possible to compute the distance"
            " matrix"
        )
        raise TooFewRoutes
