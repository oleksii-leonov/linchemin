...
class NrBranches(DescriptorCalculator):
    ...
    def compute_descriptor(
        ...
        if isinstance(graph, BipartiteSynGraph):
            ...
            logger.error(
                f"{type(graph)} is not supported. Only BipartiteSynGraph and"
                " MonopartiteReacSynGraph are accepted."
            )
...
class Branchedness(DescriptorCalculator):
    ...
    info = (
        'Computes the "branchedness" of the input SynGraph, weighting the number of'
...
class PathFinder(DescriptorCalculator):
    ...
    info = (
        "Computes all the paths between the SynRoots and the SynLeaves in the input"
        " SynGraph"
    )
...
class Convergence(DescriptorCalculator):
    ...
    info = (
        'Computes the "convergence" of the input SynGraph, as the ratio between the'
...
class AtomEfficiency(DescriptorCalculator):
    ...
    info = (
        "Computes the atom efficiency of the input SynGraph, as the ratio between the"
        " number of atoms in the target and the number of atoms in the starting"
        " materials "
    )
